...
from PyQt5.QtGui import QColor, QPixmap, QFont, QIcon, QPolygon, QMouseEvent, QDragEnterEvent, QDragMoveEvent
from PyQt5.QtWidgets import QMainWindow, QWidget, QToolBar, QToolButton, QHBoxLayout, QLabel, QApplication
from PyQt5.QtPrintSupport import QPrintDialog, QPrinter
from PyQt5.Qwt import QwtPicker
...
class Plot(Qwt.QwtPlot):
    ...
    def __init__(self, parent=None):
        ...
        #self.getSelection.connect(self.getPoints)
    ...
    def setDamp(self, damping):
        ...
        for i in range(ArraySize):
            ...
            if (amplitude[i] > amax):
                amax = amplitude[i]
                fmax = frequency[i]
        f3 = frequency[i3] - (frequency[i3] - frequency[i3 - 1]) / (amplitude[i3] - amplitude[i3 - 1]) * (
                    amplitude[i3] + 3)
...
class myPicker(QwtPicker):

    getSelection = pyqtSignal(QPolygon)

    def eventFilter(self, obj, event):
        #print(f"event filter event type {event.type()}")
        if event.type() == QEvent.MouseButtonRelease:
            ev1 = QMouseEvent(QEvent.MouseButtonPress, event.pos(), event.button(), event.buttons(), event.modifiers())
            print(f"exit mouse")
            pos = self.selection()
            self.getSelection.emit(pos)
            QwtPicker.transition(self, ev1)
            self.reset()
            return True

        return super(myPicker, self).eventFilter(obj, event)

    def __init__(self, *args):
        QwtPicker.__init__(self, *args)
        self.getSelection.connect(self.myPoints)
        self.installEventFilter(self)

    def myPoints(self, polygon):
        if not polygon.isEmpty():
            if polygon.size() < 2:
                print(f"type got is polygon {polygon.point(0)}")
            else:
                print(f"type got is polygon {polygon.point(0)} and index 1 {polygon.point(1)}")
...
class MainWindow(QMainWindow):
    def __init__(self, *args):
        ...
        self.d_picker = myPicker(Qwt.QwtPlotPicker.PolygonRubberBand, Qwt.QwtPicker.AlwaysOn,
                                          self.d_plot.canvas())
        self.d_picker.setStateMachine(Qwt.QwtPickerPolygonMachine())
        self.d_picker.setRubberBandPen(QColor(Qt.green))
        self.d_picker.setRubberBand(Qwt.QwtPicker.PolygonRubberBand)
        ...
        #self.d_picker.selected['const QPolygon &'].connect(self.my_selected)
        self.d_picker.selected.connect(self.my_selected)
