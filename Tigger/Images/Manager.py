...
class ImageManager(QWidget):
    ...
    """
        ...
        self.showMessage.emit("Reading FITS image %s" % filename, 3000)
        QApplication.flush()
        try:
            ...
            busy = None
            traceback.print_exc()
            msg = """<P>Error loading FITS image %s: %s. This may be due to a bug in Tigger; if the FITS file loads fine in another viewer,
          please send the FITS file, along with a copy of any error messages from the text console, to osmirnov@gmail.com.</P>""" % (
            ...
            self.showErrorMessage.emit(msg)
            return None
        ...
        ic = self._createImageController(image, "model source '%s'" % model if model else filename, model or image.name,
                                         model=model)
        self.showMessage.emit("Loaded FITS image %s" % filename, 3000)
