...
class ModelItem(object):
    ...
    def __init__(self, *args, **kws):
        ...
        for kw, default in self.optional_attrs.items():
            ...
            if not isinstance(value, AllowedTypesTuple):
                raise TypeError(
                    "invalid type %s for attribute %s (class %s)" % (type(value).__name__, kw, self.__class__.__name__))
        ...
        if self.allow_extra_attrs:
            ...
            raise TypeError(
                "unknown parameters %s in constructor of %s" % (','.join(list(kws.keys())), self.__class__.__name__))
        # other init
