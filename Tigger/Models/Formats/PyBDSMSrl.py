...
"""Loads a PyBDSM-format .srl file. Srl files are essentially ASCII tables with a very specific naming convention."""

# Source_id Isl_id Wave_id RA E_RA DEC E_DEC Total_flux
...
def load (filename,freq0=None,**kw):
  ...
  for line in file(filename):
    ...
    elif re.match("#(\s*[\w:]+\s+)+",line) and line.find("Source_id") > 0:
    ...
    if format and freq0:
      break;
  if not format:
    raise ValueError,"this .srl file does not appear to contain a format string"
  # call ASCII.load() function now that we have the format dict
  kw['format'] = format;
  return ASCII.load(filename,**kw)

Tigger.Models.Formats.registerFormat("Srl",load,"PyBDSM .srl file",(".srl",));
