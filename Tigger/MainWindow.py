...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent, hide_on_close=False):
        ...
        cwlo = QVBoxLayout(cw)
        cwlo.setSpacing(5)  # todo: was setMargin, not sure if this is right (gijs)
        # make splitter
    ...
    def _getFilenamesFromDropEvent(self, event):
        """
        Checks if drop event is valid (i.e. contains a local URL to a FITS file), and returns list of
    ...
    def _mergeFileCallback(self):
        if not self._merge_file_dialog:
            ...
            dialog.setFileMode(QFileDialog.ExistingFile)
            dialog.setModal(True)
            dialog.filesSelected.connect(self._currier.curry(self.openFile, merge=True))
